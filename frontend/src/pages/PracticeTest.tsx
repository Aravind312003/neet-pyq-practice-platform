...
const PracticeTest: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 flex flex-col">
        ...
        <div className="max-w-7xl mx-auto px-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-bold text-gray-900 uppercase tracking-tight">
              {type === 'year' ? `NEET PYQ ${id}` : 'Random Mock Exam'}
            </span>
          </div>
          <div className="flex items-center gap-4">
            ...
            {!isReviewMode && (
              <button onClick={() => handleSubmitTest(false)} className="text-xs font-semibold px-3.5 py-1.5 bg-blue-600 text-white rounded-lg">
                Submit Exam
              </button>
            )}
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 flex-1 w-full grid grid-cols-1 md:grid-cols-10 gap-6 items-start">
        <div className="md:col-span-7 bg-white border border-gray-100 rounded-2xl p-6 shadow-xs">
          <div className="flex justify-between items-start mb-6">
            <div className="flex gap-2">
              <span className="px-2.5 py-0.5 text-xs font-bold text-blue-700 bg-blue-50 rounded">{activeQuestion.subject}</span>
              <span className="px-2.5 py-0.5 text-xs font-semibold text-gray-600 bg-gray-50 rounded">{activeQuestion.chapter}</span>
            </div>
            <button onClick={handleToggleBookmark} className={`p-1.5 rounded-lg border ${bookmarks.has(activeQuestion.id) ? 'bg-amber-50 text-amber-500' : 'bg-white text-gray-400'}`}>
              <Bookmark className={`w-4.5 h-4.5 ${bookmarks.has(activeQuestion.id) ? 'fill-current' : ''}`} />
            </button>
          </div>

          <h3 className="text-base md:text-lg font-bold text-gray-900 leading-relaxed mb-8">
            <span className="text-blue-600 mr-2">Q{currentIndex + 1}.</span> [NEET {activeQuestion.year}] {activeQuestion.question}
          </h3>
          ...
          <div className="space-y-3.5">
            {[
              ...
              let optClass = 'border-gray-100 hover:bg-gray-50 text-gray-700';
              let badgeClass = 'bg-gray-100 text-gray-500';

              if (isSelected) {
                optClass = 'border-blue-500 bg-blue-50/40 text-blue-900 font-semibold';
                badgeClass = 'bg-blue-600 text-white';
              }

              return (
                <button key={opt.key} onClick={() => handleSelectOption(opt.key)} className={`w-full text-left px-4 py-3 border rounded-xl text-xs flex items-center gap-3 ${optClass}`}>
                  <span className={`w-5 h-5 rounded-md flex items-center justify-center font-bold font-mono ${badgeClass}`}>{opt.key}</span>
                  <span>{opt.text}</span>
                </button>
              );
            })}
          ...
          <div className="mt-10 pt-6 border-t border-gray-100 flex gap-3 justify-between">
            <button onClick={handleClearSelection} className="px-3.5 py-2 text-xs font-semibold rounded-lg border text-gray-500">
              Clear Selection
            </button>
            <div className="flex gap-2">
              <button onClick={handlePrev} disabled={currentIndex === 0} className="px-4 py-2 text-xs font-semibold rounded-lg border disabled:opacity-50">
                Previous
              </button>
              <button onClick={handleNext} disabled={currentIndex === questions.length - 1} className="px-4 py-2 text-xs font-semibold rounded-lg bg-blue-600 text-white disabled:opacity-50">
                Next
              </button>
            </div>
          </div>
        ...
        <div className="md:col-span-3 bg-white border border-gray-100 rounded-2xl p-4 shadow-xs">
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 text-center max-w-sm w-full space-y-4 shadow-2xl">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleCancelTest} className="flex-1 py-2 bg-red-600 text-white rounded-xl text-xs">Cancel Test</button>
            </div>
          </div>
        </div>
      )}
